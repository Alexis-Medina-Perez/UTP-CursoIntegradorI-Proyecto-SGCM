/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package pe.edu.sgcm.Services.Interface;

import java.util.List;
import pe.edu.sgcm.DTOs.Requests.ActualizarEstadoTareaRequest;
import pe.edu.sgcm.DTOs.Requests.ActualizarTareaRequest;
import pe.edu.sgcm.DTOs.Requests.CrearTareaRequest;
import pe.edu.sgcm.DTOs.Responses.TareaResponse;

/**
 *
 * @author alexis
 */

public interface ITareaService {

    boolean crearTarea(CrearTareaRequest request) throws Exception;

    List<TareaResponse> listarTareas() throws Exception;

    boolean actualizarEstado(ActualizarEstadoTareaRequest request) throws Exception;
    
    boolean actualizarTarea(ActualizarTareaRequest request) throws Exception;
}
